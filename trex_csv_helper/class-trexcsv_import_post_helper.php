 ...
 */
class TREXCSV_Import_Post_Helper
{
    ...
    protected function setPost($post_id)
    {
        ...
        if (is_object($post)) {
            ...
            $this->addError('post_id_not_found', __('Provided Post ID not found.', 'trex-csv-importer'));
        }
    }
    ...
    {
        $object = new TREXCSV_Import_Post_Helper();
    ...
    protected function updateMeta($key, $value)
    {
        ...
        if ($post instanceof WP_Post) {
            ...
            $this->addError('post_is_not_set', __('WP_Post object is not set.', 'trex-csv-importer'));
        }
    }
        ...
        if ($url && is_object($wp_filesystem)) {
            ...
            if (!is_wp_error($response) && $response['response']['code'] === 200) {
                ...
                if ( $body && $wp_filesystem->put_contents($filepath , $body, FS_CHMOD_FILE) ) {
                    ...
                    $this->addError('remote_get_failed', __('Could not get remote file.', 'trex-csv-importer'));
                }
