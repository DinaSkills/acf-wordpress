...
// Load Helpers
require ABSPATH . 'wp-content/themes/schweiz2.0/trex_csv_helper/class-trex_csv_helper.php';
require ABSPATH . 'wp-content/themes/schweiz2.0/trex_csv_helper/class-trexcsv_import_post_helper.php';
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class TREX_CSV_Importer extends WP_Importer {
    ...
    */
    ...
    function header() {
        echo '<div class="wrap">';
        echo '<h2>'.__('Import CSV', 'trex-csv-importer').'</h2>';
    }
    ...
    // Step 1
    function greet() {
        echo '<p>'.__( 'Choose a CSV (.csv) file to upload, then click Upload file and import.', 'trex-csv-importer' ).'</p>';
        echo '<p>'.__( 'Excel-style CSV file is unconventional and not recommended. LibreOffice has enough export options and recommended for most users.', 'trex-csv-importer' ).'</p>';
        echo '<p>'.__( 'Requirements:', 'trex-csv-importer' ).'</p>';
        echo '<ol>';
        echo '<li>'.__( 'Select UTF-8 as charset.', 'trex-csv-importer' ).'</li>';
        echo '<li>'.sprintf( __( 'You must use field delimiter as "%s"', 'trex-csv-importer'), TREX_CSV_Helper::DELIMITER ).'</li>';
        echo '<li>'.__( 'You must quote all text cells.', 'trex-csv-importer' ).'</li>';
        echo '</ol>';
        /*echo '<p>'.__( 'Download example CSV files:', 'trex-csv-importer' );
        echo ' <a href="'.plugin_dir_url( __FILE__ ).'sample/sample.csv">'.__( 'csv', 'trex-csv-importer' ).'</a>,';
        echo ' <a href="'.plugin_dir_url( __FILE__ ).'sample/sample.ods">'.__( 'ods', 'trex-csv-importer' ).'</a>';
        echo ' '.__('(OpenDocument Spreadsheet file format for LibreOffice. Please export as csv before import)', 'trex-csv-importer' );
        echo '</p>';*/
    ...
    function import() {
        ...
        if ( isset( $file['error'] ) ) {
            echo '<p><strong>' . __( 'Sorry, there has been an error.', 'trex-csv-importer' ) . '</strong><br />';
            echo esc_html( $file['error'] ) . '</p>';
            return false;
        } else if ( ! file_exists( $file['file'] ) ) {
            echo '<p><strong>' . __( 'Sorry, there has been an error.', 'trex-csv-importer' ) . '</strong><br />';
            printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'trex-csv-importer' ), esc_html( $file['file'] ) );
            echo '</p>';
            return false;
        }
    ...
    /**
    * Insert post and postmeta using `TREXCSV_Import_Post_Helper` class.
    *
    * @param array $post
    * @param array $meta
    * @param array $terms
    ...
    * @return TREXCSV_Import_Post_Helper
    */
    public function save_post($post,$meta,$terms,$thumbnail,$is_update) {
        ...
        if ($is_update) {
            $h = TREXCSV_Import_Post_Helper::getByID($post['ID']);
            $h->update($post);
        } else {
            $h = TREXCSV_Import_Post_Helper::add($post);
        }
    ...
    function process_posts() {
        $h = new TREX_CSV_Helper;

        $handle = $h->fopen($this->file, 'r');
        if ( $handle == false ) {
            echo '<p><strong>'.__( 'Failed to open file.', 'trex-csv-importer' ).'</strong></p>';
            ...
            if ($is_first) {
                ...
                if ($post_type) {
                    if (post_type_exists($post_type)) {
                        ...
                        $error->add( 'post_type_exists', sprintf(__('Invalid post type "%s".', 'trex-csv-importer'), $post_type) );
                    }
                } else {
                    echo __('Note: Please include post_type value if that is possible.', 'trex-csv-importer').'<br>';
                }
                ...
                if ($post_id) {
                    ...
                    if ( is_null( $post_exist ) ) { // if the post id is not exists
                        ...
                        if ( !$post_type || $post_exist->post_type == $post_type ) {
                            ...
                            $error->add( 'post_type_check', sprintf(__('The post type value from your csv file does not match the existing data in your database. post_id: %d, post_type(csv): %s, post_type(db): %s', 'trex-csv-importer'), $post_id, $post_type, $post_exist->post_type) );
                        }
                    }
                }
                 ...
                 * @param bool $is_update
                 */
                $post = apply_filters( 'trex_csv_importer_save_post', $post, $is_update );
                /**
                 * Filter meta data.
                 *
                 ...
                 * @param bool $is_update
                 */
                $meta = apply_filters( 'trex_csv_importer_save_meta', $meta, $post, $is_update );
                /**
                 ...
                 * @param bool $is_update
                 */
                $tax = apply_filters( 'trex_csv_importer_save_tax', $tax, $post, $is_update );
                /**
                 ...
                 * @param bool $is_update
                 */
                $post_thumbnail = apply_filters( 'trex_csv_importer_save_thumbnail', $post_thumbnail, $post, $is_update );
                 ...
                 * @param bool false
                 */
                $dry_run = apply_filters( 'trex_csv_importer_dry_run', false );
                ...
                if (!$error->get_error_codes() && $dry_run == false) {
                     ...
                     */
                    $class = apply_filters( 'trex_csv_importer_class', null );
                    ...
                    if ($result->isError()) {
                        ...
                        if (is_object($post_object)) {
                             ...
                             */
                            do_action( 'trex_csv_importer_post_saved', $post_object );
                        }
                        
                        echo esc_html(sprintf(__('Processing "%s" done.', 'trex-csv-importer'), $post_title));
                    }
                }
        ...
        echo '<h3>'.__('All Done.', 'trex-csv-importer').'</h3>';
    }
...
// Initialize
function trex_csv_importer() {
    $rs_csv_importer = new TREX_CSV_Importer();
    register_importer('csv', __('CSV', 'trex-csv-importer'), __('Import posts, categories, tags, custom fields from simple csv file.', 'trex-csv-importer'), array ($rs_csv_importer, 'dispatch'));
}
add_action( 'init', 'trex_csv_importer' );
